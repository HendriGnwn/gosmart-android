...
import com.atc.gosmartlesmagistra.model.response.RegisterTeacherResponse;
...
public class SignUpTeacherActivity extends AppCompatActivity {
    ...
    private void attemptSignUp() {
        ...
        if (cancel) {
            ...
            RegisterTeacherRequest request = new RegisterTeacherRequest(titleSelected, firstName, lastName, phoneNumber, address, email, password);

            Call<LoginSuccess> call = service.registerTeacher(request);
            call.enqueue(new Callback<LoginSuccess>() {
                ...
                public void onResponse(Call<LoginSuccess> call, Response<LoginSuccess> response) {
                    ...
                    if (response.raw().isSuccessful()) {
                        ...
                        RegisterTeacherResponse mError =new RegisterTeacherResponse();
                        try {
                            mError = gson.fromJson(response.errorBody().string(),RegisterTeacherResponse.class);
                            Toast.makeText(getApplicationContext(), mError.getMessage(), Toast.LENGTH_LONG).show();

                            if (mError.getRegisterTeacherError().getFirstName() != null) {
                                mFirstNameView.setError(mError.getRegisterTeacherError().getFirstName());
                                mFirstNameView.requestFocus();
                            }
                            if (mError.getRegisterTeacherError().getAddress() != null) {
                                mAddressView.setError(mError.getRegisterTeacherError().getAddress());
                                mAddressView.requestFocus();
                            }
                            if (mError.getRegisterTeacherError().getEmail() != null) {
                                mEmailView.setError(mError.getRegisterTeacherError().getEmail());
                                mEmailView.requestFocus();
                            }
                            if (mError.getRegisterTeacherError().getPassword() != null) {
                                mPasswordView.setError(mError.getRegisterTeacherError().getPassword());
